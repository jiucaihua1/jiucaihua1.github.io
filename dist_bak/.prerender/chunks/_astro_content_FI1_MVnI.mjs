import { M as unescapeHTML, c as renderUniqueStylesheet, k as createHeadAndContent, p as renderComponent, s as renderScriptElement, x as renderTemplate } from "./jsx-runtime_BecIScU7.mjs";
import { D as LiveContentConfigError, J as RenderUndefinedEntryError, at as AstroUserError, et as UnknownContentCollectionError, it as AstroError } from "./errors-data_21Mj7k88.mjs";
import { c as level, l as createComponent, o as VALID_INPUT_FORMATS } from "./config_C2j_Y-7t.mjs";
import { f as prependForwardSlash, p as removeBase, u as isRemotePath } from "./remote_DCWYK5x-.mjs";
import { t as createConsoleLogger } from "./console_BS3552R5.mjs";
import { t as createSvgComponent } from "./runtime_DbOvlAcr.mjs";
...
var ImmutableDataStore = class ImmutableDataStore {
	...
	*/
	static async fromModule() {
		try {
			const data = await import("./_astro_data-layer-content_D13upb0s.mjs");
...
function createGetCollection({ liveCollections, logger }) {
	return async function getCollection(collection, filter) {
		...
		if (await store.hasCollection(collection)) {
			const { default: imageAssetMap } = await import("./content-assets_BRaG8HKx.mjs");
			const result = [];
...
async function updateImageReferencesInBody(html, fileName) {
	...
	const { getImage } = await import("./_virtual_astro_get-image_D7Vfw9Pr.mjs");
...
function createRenderEntry({ logger }) {
	return async function renderEntry(entry) {
		...
		if (entry.deferredRender) try {
			const { default: contentModules } = await import("./content-modules_CoOPiwqC.mjs");
